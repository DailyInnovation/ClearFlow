...
import { Plus, ChevronDown, Trash2, BarChart2, Crown, RotateCcw, Check, Mail } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useChecklistStore } from '../store/useChecklistStore';
import { usePremiumStore } from '../store/usePremiumStore';
import { ItemRow } from './ItemRow';
import { PaywallModal } from './PaywallModal';
import { LegalModal, LegalView } from './LegalModal';
...
export function KitList({ onShowAnalytics }: KitListProps) {
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (newKitName.trim()) createKit(newKitName.trim());
    setNewKitName('');
    setIsCreating(false);
  };
  ...
  return (
    <>
      <div className="min-h-screen bg-background text-foreground flex flex-col max-w-2xl mx-auto w-full">
                      ...
                      >
                        ...
                        <div className="px-4 py-3 border-t border-border bg-background/60">
                          {addingToKitId === kit.id ? (
                            <form onSubmit={(e) => handleAddItem(e, kit.id)} className="flex gap-2">
                              <input
                                ref={addItemInputRef}
                                type="text"
                                value={newItemText}
                                onChange={(e) => setNewItemText(e.target.value)}
                                onBlur={() => { if (!newItemText.trim()) setAddingToKitId(null); }}
                                onKeyDown={(e) => { if (e.key === 'Escape') { setAddingToKitId(null); setNewItemText(''); } }}
                            ...
                            <button
                              onClick={() => { setAddingToKitId(kit.id); setNewItemText(''); }}
        ...
        {/* Pro upgrade strip — only when not premium */}
        {!isPremium && (
          <div className="mx-4 mb-4 bg-card border border-border rounded-xl p-4 flex items-center justify-between gap-4">
        ...
        {/* Footer */}
        <footer className="flex items-center justify-center gap-4 py-4 px-6 border-t border-border/50">
          <button
